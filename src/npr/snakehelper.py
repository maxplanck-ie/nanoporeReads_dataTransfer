...
def guppy_basecalling(config, cmdlinef, logf):
    pod5dir = os.path.join(config["info_dict"]["flowcell_path"], "pod5")
    cmd = (
        [config["guppy_basecaller"]["base_calling_cmd"]]
        + [
            "-i",
            pod5dir,
            "-s",
    ...
    if barcode is True:
        cmd = (
            cmd
            + ["--barcode_kits", config["bc_kit"]]
            + config["guppy_basecaller"]["base_calling_barcode_options"].split(" ")
        )
    else:
        cmd = cmd + ["--trim_strategy", "dna"]
    if config["info_dict"]["protocol"] == "rna":
        cmd = cmd + config["guppy_basecaller"]["base_calling_RNA_options"].split(" ")
    with open(logf, "a") as f:
