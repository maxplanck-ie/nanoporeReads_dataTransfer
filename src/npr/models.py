    ...
    with open(modelfile) as f:
        for line in f:
            if dataStatus:
                ...
                if len(_split) == 4:
                    _lis.append([_split[0], _split[1], _split[2]])
                elif len(_split) == 5:  # included barcodes.
                    _lis.append([_split[0], _split[1], _split[3]])
        ...
        if "hac" in model:
            ...
            if "prom" in model:
                supmod_prom = os.path.join(
                    modeldir, model.replace("hac", "sup") + ".cfg"
                )
                supmod_reg = os.path.join(
                    modeldir, model.replace("hac", "sup").replace("_prom", "") + ".cfg"
                )
                ...
                supmod = os.path.join(modeldir, model.replace("hac", "sup") + ".cfg")
            if os.path.exists(supmod):
                if kit in modeldic[flowcell]:
                    bps_present = int(
                        modeldic[flowcell][kit].split("_")[3].replace("bps", "")
                    )
