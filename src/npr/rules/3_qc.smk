...
rule qc_fastqc:
    ...
    params:
        odir=lambda wildcards: "FASTQC_Project_{}/Sample_{}/".format(
            wildcards.project, wildcards.sample_id
        ),
    threads: 16
    log:
...
rule qc_multiqc:
    input:
        pycoQc=expand_project_path(
            "FASTQC_Project_{project}/Sample_{sample_id}/{sample_name}_pycoqc.json"
        ),
        porechopQc=expand_project_path(
            "FASTQC_Project_{project}/Sample_{sample_id}/{sample_name}_porechop.info"
        ),
        fastqc=expand_project_path(
            "FASTQC_Project_{project}/Sample_{sample_id}/{sample_name}_fastqc.html"
        ),
        krakenQC=expand_project_path(
            "FASTQC_Project_{project}/Sample_{sample_id}/{sample_name}_kraken.report"
        ),
    output:
...
rule qc_finalize:
    input:
        multiqc=expand_project_path(
            "FASTQC_Project_{project}/multiqc/multiqc_report.html"
        ),
    output:
