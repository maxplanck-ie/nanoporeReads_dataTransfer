...
def gpu_available():
    ...
    if res.returncode == 0:
        ...
        print(
            "[red]{} caused an error while checking GPU status: {} [/red]".format(
                cmd, res.returncode
            )
        )
        # sys.exit(1)
...
rule basecalling:
    ...
    output:
        touch("flags/1_basecalling.done"),
    log:
    ...
    run:
        ...
        if params.basecaller == "dorado":
            ...
            cmd = dorado_basecalling(params.cfg, params.cmdline, log.log)
        elif params.basecaller == "guppy":
            print("[yellow] run guppy [/yellow]")
            cmd = guppy_basecalling(params.cfg, params.cmdline, log.log)
        else:
