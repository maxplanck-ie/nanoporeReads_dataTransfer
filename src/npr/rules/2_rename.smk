...
rule rename_files:
    ...
    output:
        ...
        fastqs=expand_project_path(
            "Project_{project}/Sample_{sample_id}/pass/{sample_name}.fastq.gz"
        ),
        summaries=expand_project_path(
            "Project_{project}/Sample_{sample_id}/sequencing_summary.txt"
        ),
    log:
    ...
    run:
        with open(log.log, "w") as logfile:
            if config["bc_kit"] == "no_bc":
                ...
                sampleid_dir = os.path.join(project_dir, "Sample_" + sample_id)
                pass_out = os.path.join(
                    sampleid_dir, "pass", samDic["Sample_Name"] + ".fastq.gz"
                )
                fail_out = os.path.join(
                    sampleid_dir, "fail", samDic["Sample_Name"] + ".fastq.gz"
                )
                ...
                passdir = os.path.join(sampleid_dir, "pass")
                faildir = os.path.join(sampleid_dir, "fail")
                if not os.path.exists(passdir):
                    os.mkdir(passdir)
                if not os.path.exists(faildir):
                    os.mkdir(faildir)
                logfile.write("Passing fastq files.\n")
                passlist = glob.glob(os.path.join("fastq", "pass", "*fastq.gz"))
                ...
                for sample_id in config["data"]["samples"]:
                    ...
                    if Path(ss_file).exists():
                        ...
                        logfile.write(
                            "Some barcodes from sample sheet seem to be missing in data: {}".format(
                                ss_file
                            )
                        )
