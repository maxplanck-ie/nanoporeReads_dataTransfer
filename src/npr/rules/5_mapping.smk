    ...
    run:
        with open(log.log, "w") as logfile:
            ...
            if config["info_dict"]["organism"] in list(config["genome"].keys()):
                ...
                logfile.write("{}\n".format(post))
                if pref:
                    analysis_dir = os.path.join(final_path, "Analysis_" + p)
                    if os.path.exists(analysis_dir):
                        shutil.rmtree(analysis_dir)
                    logfile.write("Making {}\n".format(analysis_dir))
                    os.mkdir(analysis_dir)
                    genome_analysis_dir = os.path.join(
                        analysis_dir, config["info_dict"]["protocol"] + "_" + genome
                    )
                    ...
                    for sample in config["data"]["samples"]:
                        ...
                        # postsample.append('-')
                        mapCmd = pref + post
                        logfile.write("Mapping command invoked:\n")
                        logfile.write(" ".join(mapCmd) + "\n")
                        # Run alignment.
                        sp.check_call(" ".join(mapCmd), shell=True)
                        # index bam file.
                        ixCmd = ["samtools", "index", "-@", "25", bamfile]
                        ...
                        qcCmd = [
                            ...
                            grab_seqsummary(
                                os.path.join(
                                    final_path, project_dir, "Sample_" + sample
                                )
                            ),
                            "-a",
                            bamfile,
